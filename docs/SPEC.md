...
- **刻み（旧・表示単位）**: 期間から自動で決まる。**月を選択 = 日ごと / 全期間 = 月ごと**の 2 分岐。
  いま何で刻んでいるかはグラフ右上に文字で出すだけで、切替 UI は置かない。
  設計案にあった「62 日以下 = 日ごと」の規則は、期間指定 UI がないため採らない（UI-SPEC §5-5）。
  - **廃止**: 「明細」（1 件ずつの折れ線）と「年別」。明細の役割は棒タップで下に出る一覧が果たす。
- **チャート集計（AggregatedPoint）**: レコードを刻みごとの日付キー（日ごと = `startOfDay` / 月ごと = 月初日）に丸めて `profit`（Σ netProfit）を合算。棒グラフ。
