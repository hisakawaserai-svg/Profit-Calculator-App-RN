...
- **対象**: `isSold == isSoldMode` のレコード。加えて検索文字列・選択月フィルタ適用後のもの。
...
- **対象レコード**: `isSold == true` かつ `saleDate != nil` のみ（出品中は一切含まれない）。
...
- **指標切替**: 「売上金額」（青）/「純利益」（緑）。
- **タップ/ドラッグ**: X 座標から最も近い集計点を選択 → ツールチップ（日付・売上・利益）と下部に内訳リスト（DisclosureGroup で商品情報＋費用内訳を展開）。内訳は指標に応じて売上額 or 純利益の降順ソート。
