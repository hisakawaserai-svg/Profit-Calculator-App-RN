...
- **商品写真（1 件 1 枚）は `docs/SPEC-V5.md` で確定した（2026-08-12・実装済み）。**
  `sale_records` に列が 1 つ増える（§1）。**画像そのものは DB に入れず、ファイルとして持つ**ので、
  「記録を消したら実体も消す」責任が repository 側に増えている（SPEC-V5 §1.5）。
  計算式・CSV の列・絞り込みは変わらない。
...
| `siteName` | String | — | `''` | **RN 版で追加（SPEC-V3 §1.5.1）**。販売サイト名の「写し」。プリセットの id は持たない。空文字 = 未設定。計算にも `buildWhere` にも入らない |
| `photoFileName` | String | **○（NULL 許容）** | なし（NULL） | **RN 版で追加（SPEC-V5 §1.1）**。商品写真のファイル名（`"<uuid>.jpg"`）。NULL = 写真なし。**画像そのものは入れず、実体はドキュメントディレクトリ配下の `photos/` にファイルとして置く**（SPEC-V5 §1.2 / §1.3）。CSV には出さない（SPEC-V5 §5） |
...
- **`photoFileName` について（SPEC-V5）**: **この列だけ NULL 許容**にしてある（他の追加列は空文字・既定値で未設定を表す）── 写真は「無い」と「空の写真」の区別が意味を持たないため。既存行への追加は `drizzle/0005_dry_ted_forrester.sql` の `ALTER TABLE ... ADD COLUMN` のみで、**バックフィルをしない**（既存の記録に写真は無いので埋める値がない。SPEC-V5 §4）。インデックスは張らない（絞り込みにも並べ替えにも使わない）。**DB の列と実体（ファイル）の寿命を一致させるのは repository の責務**で、記録の削除・写真の差し替えのときに古いファイルを消す（SPEC-V5 §1.5）。
...
- **行の写真（SPEC-V5 §2.3 / 採用案 `41a`）**: 行の左に**写真の有無にかかわらず 56pt の枠を常設**する（無い記録は薄い枠）。写真のある行だけサムネを出すと商品名の左端が行ごとに揺れ、写真の無い記録の方が多い前提なので揺れる側が多数派になるため。行の高さは 56 ＋ 上下 13 = 82pt（タグの付く行だけ 3 段目ぶん伸びるのは従来どおり）。データタブの「選択日の一覧」も同じ `RecordRow` なので同時に反映される。
...
- **種別フィルタ（SPEC-V2 §4.2）**: 合計行の右端のチップで「すべて → 不用品 → 仕入品」を巡回。リスト本体と合計行の両方に効く（**検索も 2026-08-22 から合計行に効く**。SPEC-V4 §4.5 の改訂）。並び替えの「純利益 ↑↓」は「収支 ↑↓」に改称。
...
- **RN 版の追加（SPEC-V5 §2.1 / §2.2）**: **商品名の直後・レシートカードの外側**に写真（全幅 × 196pt）。押すと全画面（ピンチ拡大は入れない。SPEC-V5 決定 §6-5）。写真が無いときは節ごと出さず、**商品名の行の右端の写真アイコン**が足す口になる（押すと編集フォームが開く。SPEC-V5 決定 §6-4）。
...
- **RN 版の追加（SPEC-V5 §3.1〜§3.3）**: **写真の欄を商品名の上**に置く（見出し「写真（任意）」・高さ 88pt）。カメラロールから 1 枚選び、差し替え・削除ができる。**カメラは起動しない**（SPEC-V5 決定 §6-2）。**実体はカメラロールから選んだ瞬間にファイルとして書かれ、DB の列に載るのは「保存」を押した瞬間**なので、その間にできた「どこからも指されないファイル」はフォームが閉じるときに自分で消す（SPEC-V5 §1.5）。
...
**RN 版で追加される state（SPEC-V5 §3.1 / §1.5）**: 記録フォームの `photoFileName`（null = 写真なし）と、
**開いている間に新しく書かれた写真ファイルの控え**（`createdPhotos`。保存・取り消しのときに、
列に載らなかったものを消すために持つ）。詳細画面には全画面表示の開閉（`viewerOpen`）が増える。
計算タブは写真の state を持たない（SPEC-V5 §3.4）。
...
- **期間フィルタ**: 期間シートで選んだ期間との一致。**期間は 3 値**（全期間 / 年 `"YYYY"` / 月 `"YYYY-MM"`。§6.2 / `src/logic/period.ts`）。期間指定（範囲）はない。初期値は今月。
...
- **期間**: 記録タブと同じ期間シートで選ぶ（UI-SPEC §1.5）。**型は 3 値**（SPEC-V3 §5.5 の改訂で「年」を追加。実装済み）:

  | 値 | 意味 | SQL の条件 |
  |---|---|---|
  | `null` | 全期間 | 期間条件を組まない |
  | `"YYYY"` | その 1 年 | `substr(販売日, 1, 4) = 期間` |
  | `"YYYY-MM"` | その 1 か月 | `substr(販売日, 1, 7) = 期間` |

  - 保存形式が `"YYYY-MM-DDTHH:mm:ss.SSS"` で先頭から年・月・日と並ぶので、**期間キーはそのまま日付の先頭一致**になる。年を足しても条件の形は変わらず、切り出す長さが変わるだけ（`logic/period.ts` の `periodKeyLength`）。
  - **型の判定・変換は `src/logic/period.ts` の純粋関数に閉じる**（`periodKind` / `shiftPeriod` / `periodMonthRange` / `canShiftPeriod`）。画面やリポジトリで文字列の長さを見て分岐しない。
  - 初期値は今月。月バーの ◀▶ は**表示中の期間を 1 つ前後に動かす**（月なら前後の月・年なら前年翌年）。無効化の規則は UI-SPEC §5-14。
...
- **刻み（旧・表示単位）**: 期間から自動で決まる。**月を選択 = 日ごと / 年・全期間 = 月ごと（全期間で対象の月数が 36 を超えたら年ごと）**の 3 分岐。年（12 か月）は閾値に触れないので必ず月ごと。
