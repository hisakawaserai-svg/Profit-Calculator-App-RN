...
#### 実績
「はじめての売却」「累計利益 10 万円」「得意分野」など 46 種類。難易度に応じてブロンズ〜レジェンドの段位が付き、達成するとトーストで知らせます。数字を追うだけでは続かないので、**続けること自体に手応えを持たせる**ために入れた機能です。
...
#### 使いかた
6 ページ（計算 / 記録 / 売る / データ / 残す / ことば）の説明を、図つきで収録。各画面の右上の「？」からは、その画面の説明だけが開きます。

#### チュートリアル
初回起動時に 8 ステップのチュートリアル。設定からいつでも見直せます。
...
- **vitest** — テスト 63 ファイル（うち 34 が `src/logic/` の計算・集計ロジック）
...
仕様は [docs/SPEC.md](docs/SPEC.md) と、以降の改訂 `docs/SPEC-V2.md` 〜 `docs/SPEC-V9.md` にあります。
