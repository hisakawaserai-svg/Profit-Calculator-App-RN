// 「ベルを最後に開いた日時」のうち、保存先に依存しない部分。lastBackupAt.ts と同じ形。
//
// **これは設定ではなく端末の履歴**（利用者が選ぶ値ではない）。未読ドット（hasUnreadBell）が、
// 今日すでに開いたか・履歴より後に開いたかを見るために使う。通知一覧そのものは毎回その場で
// 計算するので、既読・未読を項目ごとに持たない。
