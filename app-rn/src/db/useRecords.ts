 ...
 * @param summaryFilter 合計行の絞り込み。**省略時は filter と同じ**で、記録タブは省略する。
 *
 *                      かつては記録タブが「検索だけ除いたもの」を渡していた（検索は探す操作で
 *                      見る対象の限定ではない、という SPEC-V4 §4.5 の表）。**実機で破綻したので
 *                      やめた** ── 検索に 1 件も合わないとき、一覧は空なのに合計行だけ数字が残る。
 *                      期間・状態・種別・販売サイト・タグは元から一覧と同じ条件で、
 *                      検索だけが例外だったので、その例外を無くして 1 本に揃えた。
 *
 *                      口そのものは残してある ── 「一覧とは別の母集団を数える合計行」が要る
 *                      画面が出たときに、また同じ分岐を画面側に書き足さずに済むように。
 */
