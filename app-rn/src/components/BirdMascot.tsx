...
/**
 * 寝息の Z（頭の右から左上へ斜めに立ちのぼる 3 つ）。100×100 基準。
 *
 ...
 *
 * 位置は次の 2 つを満たしてある。
 * - シーンの丸（cx50 cy50 r48）の内側（はみ出すと円の外に Z だけが浮いて見える）
 * - 白い体に重ねない（眠りシーンでは zColor が既定の白なので、体に載ると消える）
 *
 * **ただし眠りシーンの月（cx75 cy25 r13）とは重なる。** 頭の横に小さい Z を置ける
 * 高さ（y27〜32）で体を避けられるのは x71 以降しかなく、そこは月の内側だから ──
 * 右から左上へ流す形と月の位置は両立しない。showScene=false（記録タブの使い方）では
 * 月を描かないので影響しない。シーン付きで使うときは月を左へ移すこと。
 */
const SLEEP_ZS = [
  { d: zPath(71.5, 27, 4.5), strokeWidth: 1.4 },
  { d: zPath(60.5, 15, 6.5), strokeWidth: 1.7 },
  { d: zPath(48.5, 5, 8.5), strokeWidth: 2.1 },
] as const;
