...
type Props = {
  ...
  value: number | null;
  /**
   * 記録が持つ名前の写し（§1.5.1 / 0012）。**渡したときはバッジを名前で決める** ──
   * 額や率で決めると、同じ額のプリセットが 2 件あるときに並び順で先の 1 件が勝ち、
   * **選んだものと違うプリセットの札が出る**。手で額・率を変えても札が残るのも同じ理由。
   *
   * 渡すのは**記録に紐づく 2 つの欄**だけ:
   *   - 販売サイト（`site_name`）… 空文字は「選んでいない」。値では引き直さない
   *   - 送料（`shipping_name`。0012）… 空文字は `undefined` に落として値の逆引きへ
   *     （写しを持つ前の記録の見た目を変えないため。logic/preset の presetNameForLookup）
   *
   * 記録を持たない欄（計算タブ）は渡さない ── 引く名前がそもそも無い。
   */
  selectedName?: string;
  /**
