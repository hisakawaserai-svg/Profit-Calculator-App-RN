// 行の右端の「タグ」ボタン（SPEC-V3 §4.1 / 設計案 26a-2）。押すと選択シートが下から出る。
//
//     [ 送料              210  🏷▾  🖩 ]
//
// 置き場所は数値欄の右端・電卓ボタンの左隣（§4.1）。「この欄に値を入れる手段」が
// 1 か所にまとまり、行の高さ（60px / 伝票カードは詰めた値）は変わらない。
//
...
/**
 * タグボタンを置かない行に、置いた行と同じ幅を空ける詰め物（設計案 26a-2 の派生）。
 *
 * 伝票カード・入力カードは「販売価格から下へ引いていって結果に至る」流れを縦に読む面なので、
 * 行ごとに金額の右端がずれると、その流れが桁の食い違いに見える。ボタンの有無で
 * 数字の位置を動かさないために、同じ `styles.button` の幅だけを取る空の View を置く。
 */
export function PresetTagSlot() {
  return <View style={styles.button} accessibilityElementsHidden importantForAccessibility="no-hide-descendants" />;
}
...
const styles = StyleSheet.create({
  button: {
    ...
    // バッジ 24 / アイコン 22 のどちらが入っても、右隣の電卓ボタンの位置は動かない
