...
export const ja = {
  ...
  calculator: {
    ...
    /**
     * 合計を欄へ書き戻すボタン（§7.1）。**電卓と梱包材シートが同じキーを引く**ので、
     * ここを変えると両方が同時に変わる（片方だけ違う語になることがない）。
     *
     * 「入れる」から変えたのは、下端の塗りボタンに移って**確定の口**になったため。
     * ヘッダのリンクだった頃は「（この数字を欄へ）入れる」と読めたが、
     * キーパッドの真上に大きく置くと何をどこへ入れるのかが読みにくい。
     */
    submit: '決定',
  ...
  list: {
    ...
    filter: '絞り込み',
    filterClear: '絞り込みを解除',
    /** 空表示の 3 種。**絞り込みの結果ゼロ・期間にゼロ・そもそも記録が無いのを別の文にする** */
    filterEmptyTitle: '条件に合う記録がありません',
    noRecordsTitle: 'この期間の記録はありません',
    /**
     * その状態（売れた記録 / 出品中）にまだ 1 件も無いときの見出し。**状態ごとに分ける。**
     *
     * noRecordsTitle（「この期間の」）と分けるのは、**期間を動かしたこともない人に
     * 期間の話をしても通じない**ため ── 自分の操作のせいで隠れているのかと読めてしまう。
     * さらに「まだ記録がありません」と一括りにもしない ── 出品中に記録があるのに
     * 売れた記録のタブで「記録がありません」と出ると、片方にはある事実と食い違う。
     */
    firstSoldRecordTitle: 'まだ売れた記録がありません',
    firstListedRecordTitle: 'まだ出品中の記録がありません',
    /** ボタンの語は {{button}} で差し込む（help.items.calc-to-record のコメント参照） */
    noRecordsBody: '左下の「＋ {{button}}」を押すと記録できます',
  ...
  form: {
    ...
    /**
     * 目標の節の逆算行から、出た額を販売価格の欄へ入れるボタン。
     * **「入れる」まで言い切る** ── 「計算する」だと押した先で何が変わるのか読めない
     * （押すと欄の値が書き換わり、伝票の利益がその場で動く）。
     */
    applyRequiredPrice: 'この価格を販売価格に入れる',
    /** 日付の節。出品中と売れた記録で欄名が変わる */
    listedDate: '出品日',
    soldDate: '販売日',
    dateSection: '{{label}} {{date}}',
    /**
     * 出品中の日付の節の畳んだ見出し（UI-SPEC §1.3-12 の改訂）。
     *
     * **「売れた」の話がこの節にあると畳んだままで分かるようにする。**
     * 元は出品日だけを出していたので、売れたに切り替えたい人がこの節を開く理由を
     * 見つけられなかった（実機の指摘）。日付は従来どおり読める。
     */
    dateSectionListed: '{{label}} {{date}}・{{notSold}}',
    /** 売れた日がまだ無いこと。畳んだ見出しと、節の中の切替行の両方で使う */
    notSoldYet: 'まだ売れていません',
    today: '今日（{{date}}）',
  ...
  duplicate: {
    title: '複製する記録を選ぶ',
    note: '商品名・種別・経費・販売価格・写真・メモ・タグ・目標を引き継ぎます。日付は今日から、状態は出品中から始まります。',
    recentSection: '最近の記録',
    allSection: 'すべての記録',
    showAll: 'すべての記録を見る',
    tagFilter: 'タグで絞る',
  ...
  conclusion: {
    ...
    /**
     * 2 行目は**開く先の画面名から始める**（「いくらで売る？」/「どうだった？」）。
     * 動作だけを言っていた頃は、押すと何が開くのかが行から読めなかった。
     * `numberOfLines={1}` の行なので、画面名のぶん後半は短く畳んである。
     */
    detailSafe: '「いくらで売る？」で値下げを試す',
    detailSafeWithTarget: '「いくらで売る？」で目標を保てる価格を見る',
    detailLoss: '「いくらで売る？」で値上げを試す',
    detailLossWithTarget: '「いくらで売る？」で目標に戻る価格を見る',
    detailUnpriced: '「いくらで売る？」で価格を入力する',
    ...
    soldDetailRoom: '「どうだった？」で下げられた幅を見る',
    soldDetailShortfall: '「どうだった？」で目標との差を見る',
    /** 売れた × 価格未設定。結論は出せないので、入れる口だけを言う */
    soldUnpriced: '売れた価格を入れると、残った利益が出ます',
    soldDetailUnpriced: '「どうだった？」で価格を入力する',
  },
  ...
  pricing: {
    ...
    previousPrice: '前の価格',
    /**
     * シミュレーター（§9.9 / §9.10）。**状態で出し分けない固定の名前。**
     * 機能の名前が UI のどこにも無く、呼ぶたびに活用形が変わっていた（「値下げしてみる」
     * 「値下げを試す」「値下げを試せる」）ので、名詞 1 つに固定した。
     */
  ...
  notification: {
    ...
    /** 月次振り返りと出品滞留アラートが同じ朝に重なったときの、1通にまとめた通知（合意: 2026-09） */
    combined: {
      // 日本語は単数・複数で文面が変わらない。英語（en.ts）が 1 件のとき単数形にするため、キーだけ分けてある
      osBodyOne: '{{month}}の収支は¥{{total}}でした。値下げ検討中の商品も1件あります。',
      osBodyMany: '{{month}}の収支は¥{{total}}でした。値下げ検討中の商品も{{count}}件あります。',
    },
  },
  ...
  backup: {
    ...
    photoLimitBarMin: '0',
    photoLimitFooter: '写真なしでも、記録{{records}}件・タグ{{tags}}件・よく使う値{{presets}}件はすべて新しい端末に移せます。写真だけは「写真」アプリなどに保存してください。',
    limitCancel: 'やめる',
    /** 復元（§5.4） */
    restoreSection: '復元する',
    ...
    emptyFile: '{{file}} が空です。',
    infoRowCount: '{{file}}は1行だけのファイルです。行数が違います。',
    ...
    /**
     * 検証のエラー文に出す列の名前（§3.3）。**キーは DB のカラム名に寄せてある。**
     *
     * 訳文を画面の他の場所（計算タブの「販売価格」など）と共有しない ── あちらは
     * 入力欄の見出しで、こちらは**ファイルの中の列を指し示す語**。片方の言い回しを
     * 変えたときに、もう片方のエラー文が黙って変わるのは困る。
     */
    columns: {
      recordId: '記録ID',
      itemName: '商品名',
      ...
      useWidth: '平均使用サイズ（横）',
      tagId: 'タグID',
      formatVersion: '形式のバージョン',
      createdAt: '作成日時',
      recordCount: '記録の件数',
      presetCount: 'よく使う値の件数',
      tagCount: 'タグの件数',
      recordTagCount: 'タグ付けの件数',
      photoCount: '写真の枚数',
    },
    /** 枚数・容量 */
    photoCount: '{{count}}枚',
  ...
  presetAdmin: {
    ...
    listNotePackaging: '梱包材の欄の横にあるタグの印から複数選べます。合計が欄に入ります。',
    ...
    shippingTotal: '合計',
    shippingTotalNote: '記録でこれを選ぶと、この合計が送料に入ります。',
  ...
  help: {
    ...
    items: {
      ...
      'calc-calculator': {
        ...
        body: '金額を打ち込む欄（販売価格・仕入価格・送料・梱包材・その他）の右にある青い電卓のボタンを押すと電卓が開きます。「行を足す」で何行でも積めるので、箱代とテープ代を別々に入れて合計を出す、といった使い方ができます。「決定」を押すと合計だけがその欄に入ります。手数料は率なので電卓は出ず、「−」「＋」で動かします。\n\n積んだ行は、左にスワイプすると削除できます。押すとその行が編集中になり、続けて「× 2」のように打ち足せます ── あとから最初の行だけを 2 個分にしたいときはこれを使います。行の並びは変わりません。\n\n一度「決定」を押したあとは、欄の数字をそのままにしておけば、次に電卓を開いたときも前の行がそのまま残っています。欄を手で打ち直したり、よく使う値から入れ直したりすると、その数字 1 行から始まります。',
      },
      'calc-preset': {
        title: '毎回同じ数字を打つのをやめたい',
        body: '送料の欄の横にあるタグの印を押すと、登録しておいた送料から選べます。手数料の行の横にある印は「販売サイト」を選ぶもので、押して選ぶと手数料の率とサイトの名前が同時に入り、その下にサイト名の行が出ます。\n\n選んだあとは、印の右に「✕」が出ます。送料の「✕」は送料の欄を空に戻し、販売サイトの「✕」は名前だけを消します（率はそのまま残ります）。サイト名は、その下のサイト名の行の「✕」からも消せます。\n\n梱包材の印だけは押しても札に変わりません ── いくつでも選べるので、選んだものは欄の下に名前で出ます（記録には残らず、金額を手で直すと消えます）。登録は設定タブの「よく使う値」から行います。',
        linkLabel: '登録のしかたを見る ›',
      },
      'calc-shipping-material': {
        title: '送料に箱代も一緒に入れたい',
        body: '送料の欄の横にあるタグの印を押すと、登録しておいた送料が一覧で出ます。送料に「専用資材」の代金も登録しておくと、その行だけ「送料のみ」「＋資材」の 2 つに分かれます。どちらを押すといくらになるかは押す前に読めて、押した側の金額がそのまま欄に入ります。行そのものを押したときは「＋資材」のほうが選ばれます。\n\n宅配便の箱のように、その送り方でしか使わないものを入れておく場所です。どの発送にも使う封筒やテープは、梱包材のほうに登録してください。資材の代金を登録していない送料では、この 2 つは出ません。',
      ...
      'calc-to-record': {
        ...
        /**
         * ボタンの語は **{{button}} で差し込む**（record.addFab / labels.addRecordFabLabel）──
         * ここに直に書いていた頃は、ボタンを「この内容で記録する」から直したあとも
         * 説明だけが古い語で残った。「＋」は語ではなく記号（Fab の icon）なので文の側に書く。
         */
        body: '画面の左下の「＋ {{button}}」を押すと、いま入れている数字が入った状態で記録の画面が開きます。打ち直す必要はありません。「目標から逆算」で入れていたほしい利益は、そのまま記録の目標利益として引き継がれます。',
        linkLabel: '目標利益について見る ›',
      },
      'record-new': {
        title: '売れたもの・出品したものを残したい',
        /** ボタンの語は {{button}} で差し込む（calc-to-record のコメント参照）。2 か所とも同じ 1 つのボタン */
        body: '記録タブの左下の「＋ {{button}}」を押すと、「新しく作る」と「過去の記録から複製」の 2 つが出ます。「新しく作る」を選ぶと、空の記録の画面が開きます。計算タブの「＋ {{button}}」からでも同じ画面が開きます。商品名だけは必ず入れてください。',
      },
      'record-duplicate': {
        title: '前と同じような物をまた出したい',
        body: '記録タブの「＋ 記録」から「過去の記録から複製」を選ぶと、これまでの記録の一覧が出ます。商品名でさがすことも、タグで絞ることもできます。\n\n押した記録の商品名・種別・仕入価格・販売価格・送料・手数料・販売サイト・梱包材・その他・写真・メモ・タグ・目標が入った状態で、新しい記録の画面が開きます。同じ物をもう1つ記録するときに、値段や写真を含めて打ち直さずに済みます（写真は同じ1枚を共有するのではなく、別々のファイルとして複製します）。\n\n日付は今日から、状態は出品中から始まります。複製元は売れた記録でも出品中でもかまいません。作られるのは新しい記録なので、元の記録は変わりません。',
      },
      'record-kind': {
        title: '種別（不用品と仕入品）',
      ...
      'record-preset': {
        ...
        body: '設定タブの「よく使う値」に、販売サイト・送料・梱包材を登録しておけます。一度登録すれば、次からは選ぶだけで欄に入ります。\n\nどれも欄の横にあるタグの印を押すと一覧から選べて、押した値がそのまま欄に入ります。梱包材だけは複数選べます。\n\n送料の金額はご自身で登録したもので、実際の料金は配送サービスの案内で確認してください。',
      },
      'record-preset-material': {
        title: '梱包材はどこから選ぶか',
        body: '梱包材の欄の横にあるタグの印を押すと選べます。販売サイトや送料と同じ場所ですが、こちらはいくつでも選べます。\n\n「決定」を押すと選んだ分が 1 行ずつ積まれ、その合計が欄に入ります。箱とテープを別々に登録しておけば、2 つ選ぶだけで合計が出ます。選んだものは欄の下に名前で出ます（記録には残らず、金額を手で直すと消えます）。\n\n同じものを 2 個使った日は、「電卓で続ける」を押すと選んだ分が積まれた状態の電卓が開きます。そこで「× 2」と打てば個数を掛けられます。',
      },
      'record-preset-pack': {
      ...
      'record-preset-edit': {
        title: '登録した値を直す・消す',
        body: 'よく使う値の行を押すと編集の画面が開きます。消したいときは、いちばん下の「この送料を削除」（種類によって語が変わります）を押します。\n\n登録した値を直しても、保存済みの記録の金額は変わりません。手数料の率を変えても、これまでの記録の手数料はそのままです。記録に入っているのは、そのとき入れた金額そのものだからです。',
      },
      'record-badge': {
        title: 'バッジの文字を変えたい',
        body: 'よく使う値の登録・編集の画面には、上のほうに大きなバッジが出ています。このバッジをそのまま押すと、中の文字を直せます。文字を入れるための専用の欄はありません。\n\n何も入れなければ、名前の先頭が入ります。「A4・厚さ3cm以内」なら「A4」のように、自分で読みやすい 2 文字までを入れておくと、選ぶときに見分けやすくなります。\n\nここは普通に押すだけです。長押しは記録のコピーに使っているので、ここでは効きません。',
      },
      'record-color': {
        title: '色の選びかた',
        body: 'タグとよく使う値の色は、固定の 11 色から選べます。丸は 2 つの群に分かれていて、上が「まだ使っていない色」、下が「使用中」です。編集のときは見出しが変わり、上が「選べる色」、下が「ほかのタグが使用中」（よく使う値ではその種類の名前）になります。いま自分が使っている色は、上の先頭に残ります。\n\n使用中の色も押して選べます。同じ色を 2 つに付けるのが間違いとは限らないので、止めずに「「洋服」と同じ色です」のように 1 行だけ出ます。',
      },
      'record-color-custom': {
        title: '11 色にない色を使いたい',
  ...
  helpFigure: {
    ...
    presetListNote: '設定タブの「よく使う値」に、この形で並びます',
  ...
  onboarding: {
    ...
    presetTitle: 'よく使う値は、選ぶだけで入る',
    /**
     * 登録の口は**選ぶシートの上端**を先に言う（SPEC-V3 §4.3 の拡張）。設定タブを先に置いていた頃は、
     * 初回起動でいちばん最初に「記録を中断して設定タブへ行け」と教える形になっていた ──
     * その場登録を足した意味（99bb843）が、チュートリアルにだけ届いていなかった。
     * 設定タブの「{{section}}」は残す（まとめ買い・専用資材はいまも本フォームにしかない）。
     */
    presetBody: '販売サイト・送料・梱包材は、どれも欄の横の印から選べます（梱包材だけは複数選べます）。選ぶシートの上端で、その場で登録もできます。設定タブの「{{section}}」からも登録できます。',
    simulatorTitle: '出品中でも、値下げを試せる',
    /** 末尾の 1 文は**どこから開くか**（使いかたの sell-open と同じ趣旨）。これが無いと、
        機能があることだけ伝えて場所を伝えないページになる */
    simulatorBody: '今の価格から動かして、見込みの利益をその場で確認できます。動かしても記録は変わりません。記録の詳細で、金額のカードの帯の下にある行を押すと開きます。',
    ...
    examplePackagingName1: '段ボール（小）',
    examplePackagingName2: '緩衝材',
    exampleItemName: '腕時計',
    exampleTag: 'アクセサリー',
  },
  ...
  settings: {
    ...
    /**
     * ストアの掲載ページを開く行（docs/DESIGN-REVIEW-PROMPT.md §5）。
     *
     * **これは自動のレビュー依頼とは別物で、押しても OS のレビュー画面は出ない。**
     * Google のガイドラインが In-App Review の API をボタンで起動することを禁じている
     * （割り当てに達している人には何も出ず、壊れた行になるため）ので、
     * ここはストアのページを開くだけにしてある。注記でも「ストアのページが開きます」と
     * 言い切り、レビュー画面がその場で出ると読ませない。
     */
    review: {
      label: 'レビューを書く',
      note: 'ストアのページが開きます。',
    ...
    /**
     * 外部のページへの 2 行（GitHub Pages）。**アプリの中では開かない** ──
     * 端末の既定のブラウザに渡す（画面の実装の Linking.openURL）。
     *
     * 注記で「ブラウザで開きます」と言うのは、行の右の外向きアイコンを
     * 読み上げでは拾えないため ── 目で見て分かることと、読み上げで分かることを揃える。
     */
    support: {
      label: 'サポート',
      privacy: 'プライバシーポリシー',
      note: 'どちらも端末のブラウザで開きます。',
