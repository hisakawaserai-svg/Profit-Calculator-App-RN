...
describe('UI-SPEC §1.5 データタブの語', () => {
  ...
  it('注記は年・全期間で刻みが変わることだけを言う（§1.5-6）', () => {
    expect(CHART_UNIT_NOTE).toBe(
      '年や全期間を選ぶと刻みが「月ごと」（全期間で記録が3年ぶんを超えると「年ごと」）に変わります。',
    );
  });

  /**
   * 期間は月バーが示す。見出し側に「この月の」を付けると、桁が大きいときに先に切れる。
   */
  it('合計行の見出しは期間によらず「収支」（§1.2 / §1.5-4）', () => {
    expect(periodProfitLabel('ja')).toBe('収支');
    expect(periodProfitLabel('en')).toBe('Net total');
  });
